...
```{r pickcolony, eval = T}
Nb_SA_data <- All_COLONY_data %>% filter(Scenario == "scenario_00") #need to pick one(1) for example scenario_00 is the defaults with 'quick run', scenario_08 is the original settings
...
`r bands_SA$band_upper-bands_SA$band_lower` width
`r(bands_SA$band_upper+bands_SA$band_lower)/2` mean
