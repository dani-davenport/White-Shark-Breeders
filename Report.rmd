...
COLONY_sims_list <- list(
  scenario_0 = list(run_length=1, allele_freq = NULL, dropout = 0, error_rate = 0.02, random_seed = 1234), #original quick
  scenario_1 = list(run_length=1, allele_freq = "overall", dropout = 0, error_rate = 0.02, random_seed = 1234), #original with overall allele ests
  scenario_2 = list(run_length=1, allele_freq = NULL, dropout = 0.011, error_rate = 0.001, random_seed = 1234), #rates from Wang 2018
  scenario_3 = list(run_length=1, allele_freq = "overall", dropout = 0.011, error_rate = 0.001, random_seed = 1234), #rates from Wang 2018 with overall allele ests
  scenario_4 = list(run_length=1, allele_freq = NULL, dropout = 0.004, error_rate = 0.007, random_seed = 1234), #inbetween scenario 1
  scenario_5 = list(run_length=1, allele_freq = "overall", dropout = 0.004, error_rate = 0.007, random_seed = 1234), #inbetween scenario 1 with overall allele ests
  scenario_6 = list(run_length=1, allele_freq = NULL, dropout = 0.007, error_rate = 0.014, random_seed = 1234), #inbetween scenario 2
  scenario_7 = list(run_length=1, allele_freq = "overall", dropout = 0.007, error_rate = 0.014, random_seed = 1234),
  scenario_10 = list(run_length=3, allele_freq = NULL, dropout = 0, error_rate = 0.02, random_seed = 101), #original long run
  scenario_11 = list(run_length=3, allele_freq = NULL, dropout = 0, error_rate = 0.02, random_seed = 100), #original long run
  scenario_12 = list(run_length=2, allele_freq = NULL, dropout = 0, error_rate = 0.02, random_seed = 102), #original med run, same as scenario 1. See if 2 vs 3 makes a difference here
    scenario_13 = list(run_length=2, allele_freq = NULL, dropout = 0, error_rate = 0.02, random_seed = 1234) #original
)


source("colony_script.R") #radiator had bug (crazy I know) so i had to fix it. write_colony2  is radiator::write_colony but it passes arguements through correctly
...
```{r run colony, eval = rerun_COLONY, echo=F}
...
colony_files <- c(str_extract(colony_files, "COLONY_input_scenario_[0-9]_[0-9]{4}.txt"),str_extract(colony_files, "COLONY_input_scenario_[0-9]{2}_[0-9]{4}.txt"))
...
for (c_f_i in colony_files){
  ...
  #run COLONY
  system(paste0(absolute_path, "/Executables/COLONY/colony2s.ifort.out OFN:", absolute_path, "/Executables/COLONY/", c_f_o," IFN:", absolute_path, "/Data/Processed/COLONY/", c_f_i))
...
```{r colony cleanup eval = rerun_COLONY, echo=F}
...
write_csv(Nb_SA_data, "./Results/COLONY/COLONY/ResultsSummary_final.csv")
```

## Run NeEstimator

```{r run LDNe, eval = rerun_LDNE}
...
Nb_LD_data <- read_csv( "./Results/NeEstimator/NeResultsSummary.csv")
