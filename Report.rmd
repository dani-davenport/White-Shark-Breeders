...
source("./Scripts/colony_script.R") #radiator had bug (crazy I know) so i had to fix it. write_colony2  is radiator::write_colony but it passes arguements through correctly
...
rerun_COLONY_all <- FALSE #this is very slow and only really needed for the sensitivity testing stuff
...
for (i in seq_len(length(colony_results_files))){
  ...
  res_FS <- readLines(file.path("./Results/COLONY/",  str_replace(c_f_o, ".txt", "_FS.txt")))
  FS_count_Nb_SA <- length(res_FS)-1
  
  res_HS <- readLines(file.path("./Results/COLONY/",  str_replace(c_f_o, ".txt", "_HS.txt")))
