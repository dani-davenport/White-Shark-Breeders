...
---
```{r setup, message=F, echo = F, warnings = F, include = F}
...
clear_all_previous <- FALSE # clears everything processed and results before running
...
Visualisation of datasets showing published data (top) and new data (bottom). A PCA (axis 1-2_ showing the location of individuals, highlighting differences between the published and new dataset.  
...
par(mfrow = c(1, 2))
old + labs(title="PCA of data published in origional manuscript")
new + labs(title="PCA of updated data")
```
...
### A plot comparing the before- and after-correction values for LD and SA Nb, SA Nb and any derived Nb estimates. Please note, 2014 is not included in the origional or the update manuscript, but included here for interest only. Sample size did not meet filtering cutoff.   

```{r plot_table_data, message = F, echo = F, fig.cap = "Plot comparing the before- and after-correction values for LD and SA Nb, SA Nb."}
plot_data = comparison_table %>%
  select(-c("Nb_COM_est",	"Nb_COM_SD",	"Na_Nb_ratio",	"FS_count_Nb_SA",	"HS_count_Nb_SA")) %>%
  pivot_longer(cols = starts_with("Nb_"), 
               names_to = c(".value", "Type"), 
               names_pattern = "Nb_(.*)_(.*)") %>%
  mutate(LD = ifelse(is.na(LD), 2000, LD))  %>% # set to a big number if missing 
  pivot_longer(cols = c("LD", "SA"), 
               names_to = "Measure", 
               values_to = "Value") %>%
   pivot_wider(names_from = Type, values_from = Value)
p = plot_data %>% 
  ggplot(., aes(x = Year, y = est, color = Era)) +
...
p
# save 
# ggsave(plot = p, filename = "./Figures/plot-compare.png")
```
...
This estimate should be treated with extreme caution when being interpreted.
